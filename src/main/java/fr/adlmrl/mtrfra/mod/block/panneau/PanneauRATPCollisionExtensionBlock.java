...
public class PanneauRATPCollisionExtensionBlock extends BlockExtension {

    public PanneauRATPCollisionExtensionBlock() {
    ...
    private static void placeAt(World world, BlockPos target) {
        world.setBlockState(target, fr.adlmrl.mtrfra.mod.registry.ModBlocks.PANNEAU_RATP_COLLISION_EXTENSION.get().getDefaultState(), 3);
    }
    ...
    private static void removeAt(World world, BlockPos target) {
        if (world.getBlockState(target).getBlock().data instanceof PanneauRATPCollisionExtensionBlock) {
    ...
    public ActionResult onUse2(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        for (final Direction direction : Direction.values()) {
            ...
            final BlockState anchorState = world.getBlockState(anchorPos);
            final Object anchorBlock = anchorState.getBlock().data;
            if (!(anchorBlock instanceof PanneauRATPBase)) {
                continue;
            }
            ...
            if (!world.isClient()) {
                final PanneauRATPBase panel = (PanneauRATPBase) anchorBlock;
                final BlockEntity blockEntity = world.getBlockEntity(anchorPos);
                final PanneauRATPBase.BlockEntityBase panelBlockEntity = blockEntity != null && blockEntity.data instanceof PanneauRATPBase.BlockEntityBase ? (PanneauRATPBase.BlockEntityBase) blockEntity.data : null;
                final String customTitle = panelBlockEntity == null ? "" : panelBlockEntity.customTitle;
                final String subtitle = panelBlockEntity == null ? "" : panelBlockEntity.subtitle;
                final int category = IBlock.getStatePropertySafe(anchorState, PanneauRATPBase.CATEGORY);
                MTRFRARegistry.REGISTRY.sendPacketToClient(ServerPlayerEntity.cast(player), new PacketOpenPanneauConfigScreen(anchorPos, customTitle, subtitle, category, panel.maxCategory()));
            }
