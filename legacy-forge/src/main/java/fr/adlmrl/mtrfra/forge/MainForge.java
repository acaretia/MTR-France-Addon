package fr.adlmrl.mtrfra.forge;
...
public class MainForge {
        ...
        final RepositorySource repositorySource = (RepositorySource) Proxy.newProxyInstance(
                ...
                (proxy, method, args) -> {
                    ...
                    try {
                        ...
                        throw new IllegalStateException("Failed to register the MTR France Addon built-in resource pack", exception);
                    }
                    return null;
                }
        );
    ...
    private static Pack createPack(Path modFile, Object packConstructor) throws ReflectiveOperationException {
        ...
        for (Method method : Pack.class.getMethods()) {
            if (method.getName().equals("readMetaAndCreate") && method.getParameterCount() == 7) {
                final Object resourcesSupplier = createResourcesSupplier(method.getParameterTypes()[3], modFile, id);
                return (Pack) method.invoke(null, id, title, true, resourcesSupplier, PackType.CLIENT_RESOURCES, Pack.Position.BOTTOM, PackSource.BUILT_IN);
            }
        }

        for (Method method : Pack.class.getMethods()) {
            if (method.getName().equals("create") && method.getParameterCount() == 6) {
                final Object resourcesSupplier = createResourcesSupplier(method.getParameterTypes()[2], modFile, id);
                return (Pack) method.invoke(null, id, true, resourcesSupplier, packConstructor, Pack.Position.BOTTOM, PackSource.BUILT_IN);
            }
        }
    ...
    private static Object createResourcesSupplier(Class<?> supplierType, Path modFile, String id) throws ReflectiveOperationException {
        try {
            final Class<?> pathSupplierClass = Class.forName("net.minecraft.server.packs.PathPackResources$PathResourcesSupplier");
            if (supplierType.isAssignableFrom(pathSupplierClass)) {
                return pathSupplierClass.getConstructor(Path.class, boolean.class).newInstance(modFile, true);
            }
        ...
        try {
            final Class<?> pathPackClass = Class.forName("net.minecraft.server.packs.PathPackResources");
            packResources = pathPackClass.getConstructor(String.class, Path.class, boolean.class).newInstance(id, modFile, true);
        } catch (ClassNotFoundException noPathPackResources) {

            final Class<?> folderPackClass = Class.forName("net.minecraft.server.packs.FolderPackResources");
            final Object folderPackResources = folderPackClass.getConstructor(File.class).newInstance(modFile.toFile());
            return wrapAsProxy(supplierType, folderPackResources);
        }

        return wrapAsProxy(supplierType, packResources);
    }

    private static Object wrapAsProxy(Class<?> supplierType, Object packResources) {
